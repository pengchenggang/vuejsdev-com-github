<template>
  <span style="padding: 0 5px; position: relative; top: -2px; z-index: 1;"
...
export default {
  ...
  setup (props, context) {
    ...
    // console.info('provide', props.provide)
    return {
      provide1: inject('provide1'),
      s: props.s,
      mp3Url,
      player: null,
    }
  ...
  methods: {
    ...
    playAudioHandle () {
      console.info('this.provide1.player', this.provide1.player)
      this.provide1.player.src = this.mp3Url
      console.info('this.provide1', this.provide1)
      console.info('this.provide1.currentLine', this.provide1.currentLine)
      this.provide1.currentLine = this.s
      console.info('this.provide1', this.provide1)
      console.info('this.provide1.currentLine', this.provide1.currentLine)

      // console.info('playAudioHandle')
      const speed = window.localStorage.getItem('speed') ? window.localStorage.getItem('speed') : false
      // console.info('speed', speed)
      const slowSpeed = 0.4
      // if (this.provide1.player) {
      // this.player.playbackRate = 0.5
      const playbackRate = speed === 'true' ? 1 : slowSpeed
      this.provide1.player.playbackRate = playbackRate
      this.$nextTick(() => {
        // console.info('playbackRate', playbackRate)
        // console.info('this.player.playbackRate', this.player.playbackRate)
        // console.info('aaa')
        // this.player.currentTime = 0
        this.provide1.player.play()
      })
      // }
      //  else {
      //   this.player = new Audio(this.mp3Url)
      //   // this.player.playbackRate = 0.5
      //   const playbackRate = speed === 'true' ? 1 : slowSpeed
